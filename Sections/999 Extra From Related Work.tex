...
\begin{enumerate}
    \item \textbf{Purpose for ATO and RTO} \\
    Remote control is a necessary step toward high automation levels (GoA~4). Remote control enables safe testing of autonomous functions, provides human intervention capability, and serves as a fallback mode during degraded or emergency operation of autonomous trains. It ensures safety during the transition from manually operated to fully autonomous vehicles and supports autonomy by allowing oversight and intervention from a distance :contentReference\cite{jürgensen2025rcrailvehicles}.
...
\begin{enumerate}
    \item \textbf{Purpose for ATO and RTO} \\
    The fallback system is positioned as a safety mechanism within ATO: when autonomous functions fail or become unavailable, a Remote Train Operator must be able to take over. The thesis emphasises that such fallback capability is essential for achieving high Grades of Automation and for maintaining safety during degraded-mode operation .
...
\begin{enumerate}
    \item \textbf{Purpose for ATO and RTO} \\
    Remote driving is treated as a fallback capability for automated trains, requiring stable and low-delay video so an operator can intervene when needed :contentReference.
...
\begin{enumerate}
    \item \textbf{Purpose for ATO and RTO} \\
    Remote driving is treated as a capability that supports automated operations by enabling safe telecommand and fallback intervention. It allows operators to manage vehicle preparation, shunting, and movements in controlled areas without being physically in the cab, contributing to more efficient depot operations :contentReference.
...
\begin{enumerate}
    \item \textbf{Purpose for ATO and RTO} \\
    The degraded-mode scenarios support both ATO and RTO by defining how a Remote Driver can maintain safe control when autonomy cannot be relied on. They describe when the driver must take over, how responsibility shifts, and what operational limits apply under degraded information quality :contentReference[oaicite:1]{index=1}.
...
\begin{enumerate}
    \item \textbf{Purpose for ATO and RTO} \\
    The study supports RTO by identifying which video-quality factors most affect a remote operator’s ability to detect signals and react correctly. It contributes to defining minimum visual requirements for fallback layers in high automation (GoA3–4) :contentReference[oaicite:1]{index=1}.
